...
def derive_key(password, salt=to_b64_str(urandom(16))):
    return {
            'key': to_b64_str(PBKDF2HMAC(algorithm=SHA256, length=32, salt=from_b64_str(salt), iterations=100000, backend=backend).derive(password.encode())),
            'salt': salt
            }
...
def encrypt(data, key):
    ...
    return {'ciphertext': to_b64_str(encryptor.update(padder.update(data.encode()) + padder.finalize()) + encryptor.finalize()), 'IV': to_b64_str(IV)}
...
def decrypt(encrypted_data, key):
    ...
    return (unpadder.update(decryptor.update(from_b64_str(encrypted_data['ciphertext'])) + decryptor.finalize()) + unpadder.finalize()).decode()
