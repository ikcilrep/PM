...
def verify_password(password, hash_digest): return to_b64_str(get_scrypt_derivator(
    from_b64_str(hash_digest['salt'])).derive(password.encode('utf-8'))) == hash_digest['hash']

# Returns dictionary with salt ('salt') and result of scrypt derivation for password ('hash'). 
def hash_digest(password):
    salt = urandom(16)
    return {'salt': to_b64_str(salt), 'hash': to_b64_str(get_scrypt_derivator(salt).derive(password.encode('utf-8')))}
